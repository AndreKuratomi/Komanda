...
table_1 = [{'id': 1, 'amount': 4}, {'id': 34,'amount': 5}]


def calculate_tab(some_table):
    conta = 0
    contador = 0
    while contador < len(some_table):
        conta += menu[some_table[contador].get('id')-1].get('price') * some_table[contador].get('amount')
        contador += 1
    return conta
